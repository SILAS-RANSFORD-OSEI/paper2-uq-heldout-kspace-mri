\begin{table*}[t]
\centering
\caption{Task R localization performance across frozen residual-risk thresholds.}
\label{tab:task_r_threshold_sensitivity}
\begin{tabular}{llccc}
\hline
Method & Category & $q_{85}$ AUPRC [95\% CI] & $q_{90}$ AUPRC [95\% CI] & $q_{95}$ AUPRC [95\% CI] \\
\hline
C0: Direct residual-risk predictor & Direct predictor & \textbf{0.929 [0.914, 0.944]} & \textbf{0.828 [0.801, 0.854]} & \textbf{0.623 [0.594, 0.652]} \\
...
\footnotesize
\textit{Note:} Values are mean volume-level support-weighted AUPRC with 95\% paired volume-bootstrap confidence intervals across 40 test volumes. Thresholds were fixed from the calibration split. Higher values indicate stronger localization of elevated measurement-derived residual-consistency risk. Absolute AUPRC values should not be interpreted as directly comparable across thresholds because high-risk prevalence changes with the threshold.
\end{minipage}
\end{table*}
